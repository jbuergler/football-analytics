...
For this project, event data for the UEFA Women's EURO 2025 was sourced via the `StatsBombR` package in R [@hudlNoDate]. Recently, StatsBomb was incorporated into Hudl's industry-leading ecosystem of integrated video and analytic solutions [@hudlStatsbomb2024]. They are an established provider of football event data, recording over 3,400 events per match across more than 190 competitions and thus providing a wide range of data free of charge [@hudlStatsbombNoDate]. Key functions, including `allclean()` for enriching event coordinates and pass end locations were used based on the workflow outlined in the Working with R guide [@statsbomb2022, p.25].
...
It is the way England won that makes the result worth looking at. As shown in Section 4.4, Spain pressed higher than England throughout the tournament, but in the final, England pressed more. Combining this with a pass share mainly focused on their own defensive third suggests a team that chose to absorb pressure instead of attacking themselves. By being defensively strong, they were able to limit Spain's space and get the game to penalties. Spain only scoring once from 2.14xG is surprising. Statsbomb's xG model already includes goalkeeper and defender positioning at the moment of each shot [@hudlStatsbomb2021]. England's defensive resilience explains part of that gap, but it cannot be confirmed from open event data alone. That England's win does show a pattern that is regularly observed in football research. Match outcomes are heavily influenced by randomness and may not always reflect underlying team performance [@brechotFlepp2020].
